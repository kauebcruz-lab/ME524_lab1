...
2. `calcula_medias()` estima `parametros` (`theta` e `phi` de cada time) a partir de `dados_tratados`.
3. `monte_carlo_campeonato()` chama `inicializar_acumuladores()` e, em cada uma das `B` repetições, executa `simular_jogos()`, junta o resultado aos jogos realizados (`bind_rows()`), aplica `classificar_times()` e atualiza os contadores com `atualizar_acumuladores()`. Ao final, `formatar_resultados()` produz as respostas.
